...
namespace Algorithms.DataStructures.Tree {
    ...
    public class BinarySearchTree<T, TreeType> : ITree<T, BinarySearchTreeNode<T>, TreeType>
        ...
        where TreeType : BinarySearchTree<T, TreeType> {
            ...
            /// 若无该元素返回 -1
            /// </summary>
            public int GetIndexOfElement (T key) {
                if (this.Root == null)
                    return -1;
                return this.GetIndexOfElementIn (this.Root, key);
            }
            ...
            private int GetIndexOfElementIn (BinarySearchTreeNode<T> node, T key) {
                if (node == null)
                    return 0;

                var elementsCountInLeft = node.Left?.Count ?? 0;

                if (node.Content.CompareTo (key) < 0)
                    return elementsCountInLeft + GetIndexOfElementIn (node.Right, key);
                if (node.Content.CompareTo (key) > 0)
                    return GetIndexOfElementIn (node.Left, key);
                return elementsCountInLeft;
            }
