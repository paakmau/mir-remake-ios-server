...
namespace MirRemakeBackend.Data {
    ...
    class DS_StatusImpl : IDS_Status {
        private JsonData m_statusDatas;
        ...
        public DS_StatusImpl () {
            LoadAllStatus ();
        }
        private void LoadAllStatus () {
            string jsonFile = File.ReadAllText ("Data/D_Status.json");
            m_statusDatas = JsonMapper.ToObject (jsonFile);
            res = new DO_Status[m_statusDatas.Count];
            specialAttributes=new DO_SpecialAttributeStatus[m_statusDatas.Count];
            concreteAttributes=new DO_ConcreteAttributeStatus[m_statusDatas.Count];
            for (int i = 0; i < res.Length; i++) {
                ...
                status.m_statusId = short.Parse (m_statusDatas[i]["StatusID"].ToString ());
                status.m_type = (StatusType) Enum.Parse (typeof (StatusType), m_statusDatas[i]["Type"].ToString ());
                status.m_isBuff = int.Parse(m_statusDatas[i]["IsBuff"].ToString())==1?true:false;
                s_status.m_spAttr = (ActorUnitSpecialAttributeType) Enum.Parse (typeof (ActorUnitSpecialAttributeType), m_statusDatas[i]["SpecialAttributes"][0].ToString ());
                c_status.m_conAttrArr=new ValueTuple<ActorUnitConcreteAttributeType, int>[m_statusDatas[i]["AttributeAttachArray"].Count];
                for (int m = 0; m < m_statusDatas[i]["AttributeAttachArray"].Count; m++) {
                    c_status.m_conAttrArr[m] = new ValueTuple<ActorUnitConcreteAttributeType, int>
                        ((ActorUnitConcreteAttributeType) Enum.Parse (typeof (ActorUnitConcreteAttributeType), m_statusDatas[i]["AttributeAttachArray"][m].ToString ().Split (' ') [0]),
                            int.Parse (m_statusDatas[i]["AttributeAttachArray"][m].ToString ().Split (' ') [1]));
                }
                res[i] = status;
