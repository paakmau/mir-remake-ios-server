        ...
        public ValueTuple<short,ValueTuple<ActorUnitConcreteAttributeType,int>[]>[] GetAllTitleIDAndAttributes(){
            ...
            for(int i=0;i<s_titleAttributeDatas.Count;i++){
                ...
                for(int j=0;j<s_titleAttributeDatas[i].Count;j++){
                    titleAttributes[i].Item2[j].Item1=(ActorUnitConcreteAttributeType)Enum.Parse(typeof(ActorUnitConcreteAttributeType),s_titleAttributeDatas[i][j].ToString().Split(' ')[0]);
                    titleAttributes[i].Item2[j].Item2=int.Parse(s_titleAttributeDatas[i][j].ToString().Split(' ')[1]);
                }
            }
